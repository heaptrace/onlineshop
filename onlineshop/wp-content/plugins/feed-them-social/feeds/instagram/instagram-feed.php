...
class FTS_Instagram_Feed extends feed_them_social_functions {
	...
	function fts_instagram_func($atts) {
     ...
     <?php }
			if (isset($super_gallery) && $super_gallery == 'yes') { ?>
				<div class="fts-slicker-instagram masonry js-masonry <?php if (isset($popup) && $popup == 'yes') { print 'popup-gallery '; } echo $fts_dynamic_class_name ?>" style="margin:auto" data-masonry-options='{ "isFitWidth": <?php if ($center_container == 'no') { ?>false<?php } else {?>true<?php } if ($image_stack_animation == 'no') { ?>, "transitionDuration": 0<?php } ?> }'>
					<?php }
		elseif(isset($scrollMore) && $scrollMore == 'autoscroll' || isset($height) && $height !== '') { ?>
									<div class="fts-instagram masonry js-masonry <?php if (isset($popup) && $popup == 'yes') { print 'popup-gallery '; } echo $fts_dynamic_class_name ?>" style="margin:auto;" data-masonry-options='{ "isFitWidth": true , "transitionDuration": 0 }'>
							<?php }
			else { ?>
    				<div class="fts-instagram <?php if (isset($popup) && $popup == 'yes') { echo 'popup-gallery '; } echo $fts_dynamic_class_name ?>">
    ...
    <?php }
		...
		<div class="fts-instagram-caption"><?php if (!$instagram_caption == '') { print ''.$instagram_caption.'<br/>';} ?><a href='<?php print $instagram_link ?>' class="fts-view-on-instagram-link" target="_blank"><?php _e('View on Instagram', 'feed-them-social');?></a></div>
		 <?php } ?>
		  <a href="<?php if (isset($popup) && $popup == 'yes') { print $instagram_lowRez_url; } else { print $instagram_link; } ?>" class='instaG-backg-link fts-instagram-img-link' target='_blank' title="<?php print $instagram_caption_a_title ?>">
